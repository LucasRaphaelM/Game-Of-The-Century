...
main()
{
    ...
    jogo j[5]={"minecraftimg.jpg","Minecraft","2011","Em Minecraft, os jogadores exploram um mundo aberto tridimensional","intencionalmente em blocos, pixelizado e gerado proceduralmente",
"podendo descobrir e extrair matérias-primas, ferramentas artesanais","construir estruturas ou terraplenagens.",
               "gtaVimg.jpg","Grand Theft Auto V","2013","O jogo se passa no estado ficcional de San Andreas, com a história","da campanha um jogador seguindo três criminosos e seus esforços","para realizarem assaltos sob a pressão de uma agência governamental,","o mundo aberto permite que os jogadores naveguem livremente. ",
               "eldenRingimg.jpg","Elden Ring","2022","Em Elden Ring, você faz parte dos Maculados, aportando nas Terras","intermédias com a ajuda da Graça, depois de eras exilado para além","do Mar de Névoa, esta Graça, que baniu sua raça deste lugar, é a mesma","que te chamou de volta, em busca de ajuda, e que te guiará na jornada.",
               "csGoimg.jpg","Counter Strike","2012","Global Offensive (CS:GO) é um jogo online desenvolvido pela","Valve Corporation e pela Hidden Path Entertainment, sendo uma","sequência de Counter-Strike: Source, é o quarto título principal","da franquia.",
               "theWitcher3img.jpg","The Witcher 3","2015","O jogo é um RPG de ação, e isso quer dizer que ao mesmo que tem","os elementos de um RPG, as batalhas não são por turnos, mas sim","lutas como em um jogo de ação. Geralt carrega duas espadas: uma","de prata para monstros e uma de aço para lutar contra humanos."};
    int menuJogo;
    int JogoSelecionado;
    int button;
    int a = 0;
    ...
    case 1: if (mousex() >= 65 && mousex() <= 215 && mousey() >= 585 && mousey() <= 620 && ismouseclick(WM_LBUTTONDOWN))
            ...
            if(mousex() >= 620 && mousex() <= 790 && mousey() >= 390 && mousey() <= 425 && ismouseclick(WM_LBUTTONDOWN))
            {
                BotaoNao(0,0,0,0);
                menuJogo = 1;
                delay(250);
            ...
            if(mousex() >= 810 && mousex() <= 980 && mousey() >= 390 && mousey() <= 425 && ismouseclick(WM_LBUTTONDOWN))
            {
                BotaoSim(0,0,0,0);
                delay(250);
                BotaoBrancoComBordaColorida(0,0,0,0);
...
void paginaDoJogo(char imgJogo[30], char imgFundo[30], char nomeDoJogo[20], char lancamento[5], char descricaoL1[75], char descricaoL2[75], char descricaoL3[75], char descricaoL4[75], int x1, int y1, int x2, int y2)
{
    ...
    setcolor(0);
    settextstyle(8,HORIZ_DIR,6);
    outtextxy(x1+401, y1+282, "LANÇAMENTO:");
    ...
    setcolor(0);
    settextstyle(8,HORIZ_DIR,6);
    outtextxy(x1+401, y1+426, "DESCRIÇÃO:");
...
void votacao(int x1, int y1, int x2, int y2)
{
    ...
    outtextxy(x1+90,y1+585, "Votar");
    outtextxy(x1+235,y1+585, "Voltar");
}
